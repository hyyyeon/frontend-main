...
const authenticateToken = (req, res, next) => {
  ...
  if (!token) return res.sendStatus(401); // 토큰이 없으면 403 Forbidden


  jwt.verify(token, secretKey, (err, user) => {
    if (err) {
      console.error('JWT 인증 실패:', err);
      return res.sendStatus(403); // 토큰이 유효하지 않으면 403 
    }
    req.user = user;
    next();
  });
...
app.delete('/delete-diary/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({ isSuccess: true, message: '다이어리 삭제 성공' });
  } catch (err) {
...
// 스티커 관련
// 모든 스티커 조회 (인증 필요)
app.get('/get-stickers', authenticateToken, async (req, res) => {
  try {
...
app.get('/get-user-stickers', authenticateToken, async (req, res) => {
  try {
    const [results] = await pool.query(`
      SELECT s.sticker_id, s.name, s.image_url, s.price, us.purchase_date
      FROM stickers s
      JOIN user_stickers us ON s.sticker_id = us.sticker_id
      WHERE us.user_id = ?
    `, [req.user.user_id]);

    res.json({ isSuccess: true, stickers: results });
  } catch (err) {
    console.error('사용자 스티커 목록 조회 오류:', err);
    res.status(500).json({ isSuccess: false, message: '서버 오류' });
  }
});
...
app.post('/buy-sticker', authenticateToken, async (req, res) => {
  ...
  try {
    // 스티커 정보 조회
    const [stickerResults] = await pool.query('SELECT * FROM stickers WHERE sticker_id = ?', [sticker_id]);
    ...
    // 사용자의 코인 수 조회
    const [userResults] = await pool.query('SELECT coin FROM user WHERE user_id = ?', [user_id]);
    ...
    // 코인 충분한지 확인
    if (user.coin < sticker.price) {
      return res.status(400).json({ isSuccess: false, message: '코인이 부족합니다' });
    }

    // 스티커 구매 처리
    await pool.query('INSERT INTO user_stickers (user_id, sticker_id) VALUES (?, ?)', [user_id, sticker_id]);
    await pool.query('UPDATE user SET coin = coin - ? WHERE user_id = ?', [sticker.price, user_id]);
...
app.post('/api/calendar', authenticateToken, async (req, res) => {
  const { date, color, stickerId } = req.body;
  const user_id = req.user.user_id;  // get user_id from token

  const query = `
    INSERT INTO calendar (user_id, date, color, sticker_id) 
    VALUES (?, ?, ?, ?) 
    ON DUPLICATE KEY UPDATE 
    color = VALUES(color), sticker_id = VALUES(sticker_id)
  `;

  try {
    await pool.query(query, [user_id, date, color, stickerId]);
...
app.get('/api/calendar', authenticateToken, async (req, res) => {
  ...
  try {
    const [results] = await pool.query('SELECT * FROM calendar WHERE user_id = ?', [user_id]);
    res.json({ isSuccess: true, calendarData: results });
  } catch (err) {
